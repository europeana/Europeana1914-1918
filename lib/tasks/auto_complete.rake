namespace :auto_complete do
  ...
  task :import => :environment do
    ...
    puts "There are now #{count} auto-complete words."
  end
  ...
  task :generate => :environment do
    ...
    num_words = ENV['COUNT'] || 1000
    
    output = `#{config.bin_path}#{config.indexer_binary_name} --config "#{config.config_file}" --buildstops #{output_file} #{num_words} contribution_core`
